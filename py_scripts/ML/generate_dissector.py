...
JSON_FILENAME = "/mnt/c/Users/aviv/Desktop/FinalProject(obj,array,bitfield)/server/dpi_output.json"
...
# Insert this **instead** of your original “Declare ProtoFields” loop in the static section:
    ...
    for field_name, info in simple_fields.items():
        ...
        # … inside the loop, before your else: …
            ...
            size = info["min_size"]
            bf_cnt = info.get("bitfields_count", 0)
            content_lines.append(f"    -- Bitfield: {safe_name}")
            content_lines.append(f"    if buffer:len() < offset + {size} then return end")
            # read the raw value
            content_lines.append(
                f"    local {safe_name} = buffer(offset, {size}):uint()"
            )
            # add the parent field
            content_lines.append(
                f"    local item = subtree:add(f_{safe_name}, buffer(offset, {size}))"
            )
            # now slice into sub-bitfields
            content_lines.append(
                f"    local bits_per = ({size} * 8) / {bf_cnt}"
            )
            content_lines.append(f"    for i = 0, {bf_cnt} - 1 do")
            content_lines.append(
                f"        local shift = ({bf_cnt} - 1 - i) * bits_per"
            )
            content_lines.append(
                f"        local mask = (1 << bits_per) - 1"
            )
            content_lines.append(
                f"        local bf_val = bit.band(bit.rshift({safe_name}, shift), mask)"
            )
            content_lines.append(
                f"        subtree:add(bf_fields_{safe_name}[i+1], bf_val)"
            )
            content_lines.append("    end")
            # finally bump the offset
            content_lines.append(f"    offset = offset + {size}\n")
            continue
